...
print(are_rows_equal([1, 2, 3, 7], [1, 2, 3, 8]))
...
#print(contains_row([[1, 2], [3, 4]], [1, 1]))

def intersection(table1, table2):
  empty_table = make_empty_table()
  table_row_count = get_table_row_count(table1)
  for i in range(table_row_count):
    row_tab1 = get_table_row(table1, i)
    boolean = contains_row(table2, row_tab1)
    if boolean:
      table = add_row_to_table(empty_table, row_tab1)
  return table

#print_table(intersection([[1, 'c'], [2, 'a'], [4, 'b']], [[4, 'b'], [1, 'c'], [1, 'a']]))